...
model_dir_source = os.path.join(root, "atdt/synthiaSEQ2cs/net1_r50_gt_aug")
model_dir_target = os.path.join(root, "atdt/synthiaSEQ2cs/net2_r50_wc_aug")

model_dir_transfer = os.path.join(root, "atdt/synthiaSEQ2cs/transfer_high_lr")
model_dir_baseline = os.path.join(root, "atdt/synthiaSEQ2cs/DA/mrnet/synthia_25000_step2.pth")
# model_dir_baseline = os.path.join(root, "atdt/gta2cs/DA/max_square/GTA5_to_Cityscapes_MaxSquare.pth")
# model_dir_baseline = os.path.join(root, "atdt/synthiaSEQ2cs/DA/adaptsegnet/synthia_65000.pth")
...
# model_dir_baseline = os.path.join(root, "atdt/synthiaSEQ2cs/DA/bdl/syn_2_city_deeplab.pth")
...
dataset = CS_test(root=data_dir, txt_file="/media/data_4t/aCardace/atdt/splits/cityscapes/val.txt", use_depth=True, 
                threshold=50, size=(1024, 512), label_size=(2048, 1024), mean=mean, std=std, rgb=False, interpolation="bicubic", encoding="synthiaSEQ2cs")
val_dl = data.DataLoader(dataset, batch_size=1, shuffle=False, num_workers=4, pin_memory=True)

dataset1 = CS_test(root=data_dir, txt_file="/media/data_4t/aCardace/atdt/splits/cityscapes/val.txt", use_depth=True,
                threshold=50, size=(1024, 512), label_size=(2048, 1024), interpolation="lanczos", encoding="synthiaSEQ2cs")
...
model_baseline = mrnet(num_classes=num_classes, use_se=True, train_bn=False, norm_style="gn", droprate=0.1).to(device)
# model_baseline = maxsq(num_classes=num_classes).to(device)
# model_baseline = maxsq(num_classes=num_classes).to(device)
# model_baseline = Deeplab(num_classes=num_classes).to(device)
# model_baseline2 = Deeplab(num_classes=num_classes).to(device)

# model_baseline = Deeplab(num_classes=num_classes).to(device)
# model_baseline2 = Deeplab(num_classes=num_classes).to(device)
# model_baseline0 = Res_Deeplab(num_classes=num_classes, layers=23).to(device)
...
def eval(adpative_model, model_baseline, val_dl, val_dl1, params):
    # set model to evaluation mode
        ...
        inverted_w = np.load("/media/data_4t/aCardace/atdt/weights/inverted_weights_synthia_seq.npy")
        print(inverted_w)
