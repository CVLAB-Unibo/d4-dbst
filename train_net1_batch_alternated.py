...
parser.add_argument('--txt_train1', default='/content/drive/My Drive/atdt/input_list_train_mixed_carla_cityscapes.txt',
                    help="Txt file containing path to training images")
parser.add_argument('--txt_train2', default='/content/drive/My Drive/atdt/input_list_train_mixed_carla_cityscapes.txt',
                    help="Txt file containing path to training images")
...
def train_epoch(model, loss_fn, dataset_dl1, dataset_dl2, opt=None, lr_scheduler=None, metrics=None, params=None):
    running_loss = utils.RunningAverage()
    num_batches = len(dataset_dl2)

    if metrics is not None:
        for metric_name, metric in metrics.items():
            metric.reset()

    iter_source = iter(dataset_dl1)

    for (xb, yb) in tqdm(dataset_dl2):
        xb = xb.to(params.device)
        yb = yb.to(params.device)

        try:
            xb_source, yb_source = next(iter_source)
        except:
            iter_source = iter(dataset_dl1)
            xb_source, yb_source = next(iter_source)

        xb_source = xb_source.to(params.device)
        yb_source = yb_source.to(params.device)

        batch_x = torch.cat((xb_source, xb), dim=0)
        batch_y = torch.cat((yb_source, yb), dim=0)
        ...
        if metrics is not None:
            for metric_name, metric in metrics.items():
                metric.add(output.detach(), batch_y)
...
def train_and_evaluate(model, train_dl1, train_dl2, val_dl1, val_dl2, opt, loss_fn, metrics, lr_scheduler, 
    ...
    batch_sample_train1, batch_gt_train1 = next(iter(train_dl1))
    batch_sample_train2, batch_gt_train2 = next(iter(train_dl2))
    ...
    if os.path.exists(ckpt_file_path):
        ...
        # model = utils.load_checkpoint(model, start_epoch=start_epoch, ckpt_dir=checkpoint_dir)[0]                                
    ...
    for epoch in range(start_epoch, params.num_epochs):
        ...
        predictions = inference(model, batch_sample_train1)
        output = F.interpolate(predictions, size=(params.label_size[1],params.label_size[0]), mode='bilinear', align_corners=True)
        plot = train_dl1.dataset.get_predictions_plot(
            batch_sample_train1, output.cpu(), batch_gt_train1)
        writer.add_image('Predictions_train_source', plot,
                            epoch, dataformats='HWC')

        predictions = inference(model, batch_sample_train2)
        output = F.interpolate(predictions, size=(params.label_size[1],params.label_size[0]), mode='bilinear', align_corners=True)
        plot = train_dl2.dataset.get_predictions_plot(
            batch_sample_train2, output.cpu(), batch_gt_train2)
        writer.add_image('Predictions_train_target', plot,
                            epoch, dataformats='HWC')

        predictions = inference(model, batch_sample_val1)
        output = F.interpolate(predictions, size=(params.label_size[1],params.label_size[0]), mode='bilinear', align_corners=True)
        plot = train_dl1.dataset.get_predictions_plot(
            batch_sample_val1, output.cpu(), batch_gt_val1)
        writer.add_image('Predictions_val_source', plot, epoch, dataformats='HWC')

        predictions = inference(model, batch_sample_val2)
        output = F.interpolate(predictions, size=(params.label_size[1],params.label_size[0]), mode='bilinear', align_corners=True)
        plot = train_dl1.dataset.get_predictions_plot(
        ...
        train_loss, train_metrics = train_epoch(
            model, loss_fn, train_dl1, train_dl2, opt, lr_scheduler=lr_scheduler, metrics=metrics, params=params)
    
        # Evaluate for one epoch on validation set
        val_metrics1 = evaluate(
            model, val_dl1, metrics=metrics, params=params)
        val_metrics2 = evaluate(
            model, val_dl2, metrics=metrics, params=params)
...
if __name__ == '__main__':
    ...
    # fetch dataloaders
    train_dl1 = dataloader.fetch_dataloader(
        args.data_dir, args.txt_train1, "train", params, use_data_augmentation=True)
    train_dl2 = dataloader.fetch_dataloader(
        args.data_dir, args.txt_train2, "train", params, use_data_augmentation=True)
    ...
    model = Res_Deeplab(num_classes=params.num_classes, use_sigmoid=True, layers=23).to(params.device)
    ...
    opt = optim.AdamW(model.optim_parameters(params.learning_rate), lr=params.learning_rate)
    lr_scheduler = torch.optim.lr_scheduler.OneCycleLR(
        opt, max_lr=params.learning_rate, steps_per_epoch=len(train_dl2), epochs=params.num_epochs, div_factor=20)
    ...
    train_and_evaluate(model, train_dl1, train_dl2, val_dl1, val_dl2, opt, loss_fn, metrics, lr_scheduler,
