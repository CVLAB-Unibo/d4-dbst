...
def train_and_evaluate(model, train_dl, val_dl1, val_dl2, opt, loss_fn, metrics, lr_scheduler, 
    ...
    for epoch in range(start_epoch, params.num_epochs):
        ...
        train_loss, train_metrics = train_epoch(
            model, loss_fn, train_dl, opt, lr_scheduler=lr_scheduler, metrics=metrics, params=params)
        ...
        current_value = list(val_metrics2.values())[0][0]
...
if __name__ == '__main__':
    ...
    # fetch dataloaders
    # params.encoding = "synthiaSEQ"
    train_dl = dataloader.fetch_dataloader(
        args.data_dir, args.txt_train, "train", params, use_data_augmentation=True, sem_depth=True)
    val_dl1 = dataloader.fetch_dataloader(
        args.data_dir, args.txt_val1, "val", params,  sem_depth=True)
    # params.encoding = "synthiaSEQ2cs"
    val_dl2 = dataloader.fetch_dataloader(
        args.data_dir, args.txt_val2, "val", params,  sem_depth=True)

    logging.info("- done.")

    # Define the model and optimizer
    # model = get_network(params).to(params.device)
    model = Res_Deeplab(num_classes=params.num_classes, layers=6).to(params.device)

    saved_state_dict = torch.utils.model_zoo.load_url(model_urls['resnet50'])
    new_params = model.state_dict().copy()
    for i in saved_state_dict:
        i_parts = str(i).split('.')
        if not i_parts[0]=='fc' or i_parts[0]=='':
            new_params['.'.join(i_parts[0:])] = saved_state_dict[i]
    model.load_state_dict(new_params)    

    # first_layer = torch.nn.Conv2d(4, 64, kernel_size=7, stride=1, padding=0,
    #                            bias=False)
    # model = torch.nn.Sequential(first_layer, model)

    opt = optim.SGD(model.optim_parameters(params.learning_rate), lr=params.learning_rate)
    ...
    train_and_evaluate(model, train_dl, val_dl1, val_dl2, opt, loss_fn, metrics, lr_scheduler,
