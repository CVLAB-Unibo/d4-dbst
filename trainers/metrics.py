...
class MSE:
    ...
    def add(self, predicted, target, **kwargs):
        ...
        if self.relative:
            norms = np.sum(((predicted - target) ** 2) / target, axis=(1, 2)) / np.maximum(
                np.sum(mask, axis=(1, 2), dtype=np.float32), self.eps
            )
        else:
...
class MAE(MSE):
    def add(self, predicted, target):
        ...
        target[~mask] = 1
        if self.relative:
            norms = np.sum(np.abs(predicted - target) / target, axis=(1, 2)) / np.maximum(
                np.sum(mask, axis=(1, 2)), self.eps
            )
        else:
            norms = np.sum(np.abs(predicted - target), axis=(1, 2)) / np.maximum(
                np.sum(mask, axis=(1, 2)), self.eps
            )
        self.errors += np.sum(norms)
...
class RMSELog(MSE):
    def add(self, predicted, target):
        ...
        norms = np.sum((np.log(predicted) - np.log(target)) ** 2, axis=(1, 2)) / np.maximum(
            np.sum(mask, axis=(1, 2)), self.eps
        )
        norms = np.sqrt(norms)
        self.errors += np.sum(norms)
...
class Threshold:
    ...
    def add(self, predicted, target):
        ...
        mask = (target > self.min_depth) & (target < self.max_depth)
        ratios = np.maximum(
            (predicted / np.maximum(target, self.eps)), (target / np.maximum(predicted, self.eps))
        )
        ratios = np.where(mask, ratios, 0)
        ratios_per_image = np.sum(
            (ratios < self.threshold) & (ratios >= 1), axis=(1, 2)
        ) / np.maximum(np.count_nonzero(ratios, axis=(1, 2)), self.eps)
