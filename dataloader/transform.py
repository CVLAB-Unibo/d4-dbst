...
class ToTensor(object):
    def __call__(self, image, label):
        ...
        return image, torch.tensor(label).squeeze()
...
class Resize(object):
    ...
    def __call__(self, image, label):
        image = F.resize(image, self.size, Image.LANCZOS)
        if self.resize_label:
            if isinstance(label, np.ndarray): 
                # assert the shape of label is in the order of (h, w, c)
                label = cv2.resize(label,(self.size[1], self.size[0]), cv2.NEAREST)
            else:
...
class RandomCrop(object):
    def __init__(self, size, padding=None, pad_if_needed=False, fill=0, label_fill=19, padding_mode='constant'):
