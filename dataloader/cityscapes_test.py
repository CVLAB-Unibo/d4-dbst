...
class CS_test(Dataset):
    ...
    def __init__(self, root, txt_file, transforms=None, 
                ...
                use_depth=False, size=(2048, 1024), label_size=(2048, 1024), rgb=True, interpolation="bicubic", encoding="cs"):

        super(CS_test, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        if self.encoding_name in ["synthiaSEQ2cs", "synthiaSEQ"]:
            gt_sem = self.encode_image_train_id(gt_sem)
        gt_sem = np.array(gt_sem)
        gt_sem = torch.from_numpy(gt_sem).type(torch.long) 
        
        # return img.transpose((2,0,1)).copy(), gt, gt_sem
        return img.transpose((2,0,1)).copy(), gt_sem, gt, self.labels_semantic[index]
    ...
    def get_predictions_plot_sem(self, batch_sample, predictions, batch_gt, batch_depth, encode_gt=False):

        num_images = batch_sample.size()[0]
        fig, m_axs = plt.subplots(4, num_images, figsize=(12, 8), squeeze=False)
        plt.subplots_adjust(hspace = 0.1, wspace = 0.1)
        # fig, m_axs = plt.subplots(2, num_images, figsize=(11, 4), squeeze=False)
        # plt.subplots_adjust(hspace = 0.02, wspace = 0.02)
        if predictions.dim() == 4:
            predictions = torch.argmax(predictions, dim=1)

        for image, prediction, gt, depth, (axis1, axis4, axis2, axis3) in zip(batch_sample, predictions, batch_gt, batch_depth, m_axs.T):
            ...
            depth = self.depth2color(depth.squeeze())
            axis4.imshow(depth)
            axis4.set_axis_off()
