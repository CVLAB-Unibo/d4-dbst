...
def update_mask_with_depth(mask, depth):
    ...
    eps = 1e-4
    b, h, w = mask.size()
    p_t = np.percentile(depth[-1].numpy(), 70)
    ...
    for i in range(depth.shape[0]-1):
        p = np.percentile(depth[i].numpy(), 70)
    ...
    # set model to evaluation mode
        ...
        with torch.no_grad():
            for i, (xb, yb, depth, name) in enumerate(tqdm(val_dl, position=0, leave=True)):
                ...
                name = "/media/data_4t/aCardace/datasets/CityScapes/gtFine_ltir_correct"
                name = f"{name}/{i}.png"
                ...
                name = name.replace('gtFine_ltir_correct', "train_ltir_correct")
                os.makedirs(os.path.dirname(name), exist_ok=True)
                img.save(name)
                # if i>10:
                #     break
...
mask = pil.open("/media/data_4t/aCardace/datasets/CityScapes/gtFine_ltir_correct/17.png")
mask = np.array(mask)
print(mask.max())
print(mask.min())
