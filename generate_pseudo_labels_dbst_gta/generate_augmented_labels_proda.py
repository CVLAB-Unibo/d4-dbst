...
model_dir_baseline = os.path.join(root, "gta2cs/DA/proto/from_gta5_to_cityscapes_on_deeplabv2_best_model.pkl")
...
dataset = CS_test(root=data_dir, txt_file=file_path, use_depth=True, threshold=50, size=(2048, 1024), label_size=label_size,  mean=np.array((0, 0, 0), dtype=np.float32), std=np.array((1, 1, 1), dtype=np.float32), rgb=True, interpolation="bilinear")
val_dl = data.DataLoader(dataset, batch_size=4, shuffle=False, num_workers=6, pin_memory=True)
...
val_dl1 = data.DataLoader(dataset1, batch_size=4, shuffle=False, num_workers=6, pin_memory=True)
...
model_baseline = DeepLab_proto(BatchNorm, num_classes=19, freeze_bn=False, restore_from=model_dir_baseline, bn_clr=True).to(device).to(device)
    ...
    # set model to evaluation mode
        ...
        with torch.no_grad():
            for i, (xb, yb, depth, name) in enumerate(tqdm(val_dl, position=0, leave=True)):
                ...
                take_classes = [4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18]
                ...
                name = "/media/data3/atdt/CityScapes/gtFine_proDA_full"
                name = f"{name}/{i}.png"
                os.makedirs(os.path.dirname(name), exist_ok=True)
                output.save(name)
                
                img = np.asarray(a_xb.cpu())
                img = val_dl.dataset.re_normalize(img*255)
                img = pil.fromarray(img)
                name = name.replace('gtFine_proDA_full', "train_proDA_full")
                os.makedirs(os.path.dirname(name), exist_ok=True)
                img.save(name)
                # if i>10:
                #     break
...
mask = pil.open("/media/data3/atdt/CityScapes/gtFine_proDA_full/2.png")
print(mask.size)
...
plt.imshow(new_mask)
# %%
img = pil.open("/media/data3/atdt/CityScapes/train_proDA_full/2.png")
print(img.size)
