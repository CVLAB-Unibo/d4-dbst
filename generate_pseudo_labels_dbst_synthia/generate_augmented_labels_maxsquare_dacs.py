...
palette = []
colors = {cs_class.train_id: cs_class.color for cs_class in synthiaSEQ2cs_encoding}
...
data_dir = "/data/lDeLuigi/datasets/"
# txt_val = os.path.join(root, "atdt/splits/cityscapes/val.txt")
ckpt_filename = "checkpoint.tar"
best_ckpt_filename = "model_best.tar"
model_dir_source = "synthiaSEQ2CS/net1_r50_gt_aug"
model_dir_target = "synthiaSEQ2CS/net2_r50_wc_aug"

model_dir_transfer = "synthiaSEQ2CS/transfer_high_lr"

# CAMBIA
# model_dir_baseline = "./synthiaSEQ2cs/DA/mrnet/stage2.pth"
model_dir_baseline = "synthiaSEQ2CS/DA/maxsquare/gta52cityscapes_maxsquarebest.pth"
...
dataset = CS_test(root=data_dir, txt_file=file_path, use_depth=True, threshold=50, size=(1024, 512), label_size=label_size, mean=mean, std=std, rgb=False, interpolation="bicubic", encoding="synthiaSEQ2cs")
val_dl = data.DataLoader(dataset, batch_size=1, shuffle=False, num_workers=4, pin_memory=True)

dataset1 = CS_test(root=data_dir, txt_file=file_path, use_depth=True, threshold=50, size=(1024, 512), label_size=label_size, interpolation="lanczos", encoding="synthiaSEQ2cs")
val_dl1 = data.DataLoader(dataset1, batch_size=1, shuffle=False, num_workers=4, pin_memory=True)

synthia_file_path = "splits/synthia_seq/train_3k_split.txt"
dataset_synthia = CS_test(root=data_dir, txt_file=synthia_file_path, use_depth=True, threshold=50, size=(1024, 512), label_size=label_size, mean=mean, std=std, rgb=False, interpolation="lanczos", encoding="synthiaSEQ")
val_dl_synthia = data.DataLoader(dataset_synthia, batch_size=1, shuffle=True, num_workers=4, pin_memory=True)
...
ckpt_transfer_file_path = os.path.join(model_dir_transfer, 'ckpt', ckpt_filename)
...
saved_state_dict = torch.load(model_dir_baseline, map_location=device)["state_dict"]
    ...
    # set model to evaluation mode
        ...
        with torch.no_grad():
            for i, (xb, yb, depth, name) in enumerate(tqdm(val_dl, position=0, leave=True)):
                ...
                try:
                    xb_synthia, yb_synthia, depth_synthia , _ = next(iter_synthia)
                except StopIteration:
                    iter_synthia = iter(val_dl_synthia)
                    xb_synthia, yb_synthia, depth_synthia , _ = next(iter_synthia)
                xb_synthia = xb_synthia.to(device)
                depth_synthia[depth_synthia > 60] = 60

                # print("xb_synthia", xb_synthia.shape)
                # print("yb_synthia", yb_synthia.shape)
                # print("depth_synthia", depth_synthia.shape)
                ...
                xb = torch.cat([xb_synthia.cpu(), xb], dim=0)
                yb = torch.cat([yb_synthia.cpu(), yb], dim=0)
                depth = torch.cat([depth_synthia.cpu(), depth], dim=0)
                prediction = torch.cat([yb_synthia.cpu(), prediction], dim=0)
                ...
                take_classes = torch.unique(yb_synthia).numpy()
                take_classes = np.delete(take_classes, np.where(take_classes == 12))
                n = len(take_classes)
                n = int((n + (n % 2)) / 2)
                ...
                name = "/data2/lDeLuigi/datasets/synthia_dbst/dacs/gtFine_maxsquare"
                ...
                name = name.replace('gtFine_maxsquare', "train_maxsquare")
                os.makedirs(os.path.dirname(name), exist_ok=True)
                img.save(name)
